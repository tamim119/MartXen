...
.ab-root { font-family: 'Plus Jakarta Sans', sans-serif; background: #fff; min-height: 70vh; }
...
.ab-hero { background: linear-gradient(135deg, #f8fafc 0%, #f0fdf4 100%); border-bottom: 1.5px solid #f1f5f9; padding: 56px 24px 44px; text-align: center; animation: ab-fadeUp 0.5s cubic-bezier(0.4,0,0.2,1) both; }
...
.ab-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px; flex-wrap: wrap; gap: 12px; }
...
.ab-add-btn { display: inline-flex; align-items: center; gap: 7px; background: #16a34a; color: #fff; border: none; padding: 10px 20px; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 0.78rem; font-weight: 700; border-radius: 100px; cursor: pointer; transition: all 0.22s; box-shadow: 0 4px 14px rgba(22,163,74,0.3); }
.ab-add-btn:hover { background: #15803d; transform: translateY(-2px); box-shadow: 0 8px 20px rgba(22,163,74,0.38); }

.ab-empty { text-align: center; padding: 64px 24px; }
.ab-empty-icon { width: 72px; height: 72px; border-radius: 50%; background: #f8fafc; border: 1.5px solid #e2e8f0; display: flex; align-items: center; justify-content: center; margin: 0 auto 18px; color: #cbd5e1; }
...
.ab-card { background: #fff; border: 1.5px solid #f1f5f9; border-radius: 20px; padding: 20px 20px 16px; transition: border-color 0.2s, box-shadow 0.2s, transform 0.2s; }
...
.ab-action-btn { padding: 5px 10px; border-radius: 8px; font-size: 0.7rem; font-weight: 700; font-family: 'Plus Jakarta Sans', sans-serif; border: 1.5px solid #e2e8f0; background: #fff; color: #64748b; cursor: pointer; transition: all 0.18s; }
...
.ab-set-default-btn { margin-top: 12px; padding-top: 12px; border-top: 1.5px solid #f8fafc; display: flex; align-items: center; gap: 6px; background: none; border-left: none; border-right: none; border-bottom: none; width: 100%; cursor: pointer; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 0.72rem; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.08em; transition: color 0.18s; }
.ab-set-default-btn:hover { color: #16a34a; }

/* ── Modal Alert ── */
.ab-alert { display: flex; align-items: flex-start; gap: 10px; padding: 11px 14px; border-radius: 11px; font-size: 0.8rem; font-weight: 500; line-height: 1.5; margin-bottom: 4px; }
.ab-alert-icon { width: 17px; height: 17px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; margin-top: 1px; font-size: 0.6rem; font-weight: 800; }
.ab-alert.error   { background: #fff1f2; border: 1.5px solid #fecdd3; color: #9f1239; }
.ab-alert.error   .ab-alert-icon { background: #fda4af; color: #9f1239; }
.ab-alert.success { background: #f0fdf4; border: 1.5px solid #bbf7d0; color: #14532d; }
.ab-alert.success .ab-alert-icon { background: #86efac; color: #14532d; }
.ab-alert-body { flex: 1; }
.ab-alert-close { background: none; border: none; cursor: pointer; padding: 0; opacity: 0.45; font-size: 0.9rem; color: inherit; transition: opacity 0.15s; flex-shrink: 0; }
.ab-alert-close:hover { opacity: 1; }

.ab-overlay { position: fixed; inset: 0; z-index: 9999; background: rgba(15,23,42,0.5); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; padding: 20px; animation: ab-fadeIn 0.22s cubic-bezier(0.4,0,0.2,1) both; }

.ab-modal { background: #fff; width: 100%; max-width: 500px; max-height: 92vh; overflow-y: auto; border-radius: 24px; box-shadow: 0 32px 80px rgba(0,0,0,0.2); animation: ab-slideUp 0.3s cubic-bezier(0.34,1.2,0.64,1) both; }
.ab-modal-head { display: flex; align-items: center; justify-content: space-between; padding: 22px 24px 18px; border-bottom: 1.5px solid #f1f5f9; position: sticky; top: 0; background: #fff; z-index: 1; border-radius: 24px 24px 0 0; }
...
.ab-modal-close { width: 34px; height: 34px; border-radius: 50%; background: #f8fafc; border: 1.5px solid #f1f5f9; display: flex; align-items: center; justify-content: center; cursor: pointer; color: #94a3b8; transition: all 0.18s; flex-shrink: 0; }
...
.ab-sec-label { font-size: 0.65rem; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: #94a3b8; margin-bottom: 10px; display: flex; align-items: center; gap: 8px; }
.ab-sec-label::after { content: ''; flex: 1; height: 1.5px; background: #f1f5f9; }

.ab-type-seg { display: grid; grid-template-columns: 1fr 1fr; border: 1.5px solid #f1f5f9; border-radius: 14px; overflow: hidden; }
.ab-type-btn { padding: 11px 0; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 0.78rem; font-weight: 700; border: none; background: #f8fafc; cursor: pointer; color: #94a3b8; display: flex; align-items: center; justify-content: center; gap: 7px; transition: all 0.2s; }
...
.ab-inp { width: 100%; padding: 11px 14px 11px 36px; border: 1.5px solid #f1f5f9; border-radius: 12px; font-size: 0.855rem; font-family: 'Plus Jakarta Sans', sans-serif; background: #f8fafc; color: #0f172a; outline: none; transition: border-color 0.2s, background 0.2s, box-shadow 0.2s; font-weight: 500; box-sizing: border-box; }
...
.ab-inp.err { border-color: #fca5a5; background: #fff1f2; }

.ab-sel-wrap { position: relative; }
.ab-sel { width: 100%; padding: 11px 36px 11px 14px; border: 1.5px solid #f1f5f9; border-radius: 12px; font-size: 0.855rem; font-family: 'Plus Jakarta Sans', sans-serif; background: #f8fafc; color: #0f172a; outline: none; appearance: none; cursor: pointer; font-weight: 500; transition: border-color 0.2s, box-shadow 0.2s; box-sizing: border-box; }
.ab-sel:focus { border-color: #16a34a; background: #fff; box-shadow: 0 0 0 3px rgba(22,163,74,0.1); }
.ab-sel.err { border-color: #fca5a5; background: #fff1f2; }
.ab-sel-arrow { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); pointer-events: none; color: #94a3b8; }

.ab-ship-chip { display: flex; align-items: center; justify-content: space-between; padding: 12px 16px; background: #f0fdf4; border: 1.5px solid #bbf7d0; border-radius: 14px; }
...
.ab-default-row { display: flex; align-items: center; gap: 12px; padding: 13px 16px; border: 1.5px solid #f1f5f9; border-radius: 14px; cursor: pointer; transition: border-color 0.18s, background 0.18s; background: #f8fafc; }
...
.ab-modal-foot { padding: 16px 24px; border-top: 1.5px solid #f1f5f9; display: flex; gap: 10px; justify-content: flex-end; background: #f8fafc; border-radius: 0 0 24px 24px; position: sticky; bottom: 0; }
.ab-modal-cancel { padding: 11px 20px; border-radius: 100px; border: 1.5px solid #e2e8f0; background: #fff; color: #64748b; font-size: 0.78rem; font-weight: 700; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; transition: all 0.18s; }
.ab-modal-cancel:hover { border-color: #94a3b8; color: #0f172a; }
.ab-modal-save { padding: 11px 24px; border-radius: 100px; background: #16a34a; color: #fff; border: none; font-size: 0.78rem; font-weight: 700; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; display: inline-flex; align-items: center; gap: 7px; transition: all 0.22s; box-shadow: 0 4px 12px rgba(22,163,74,0.3); }
...
.ab-del-modal { background: #fff; width: 100%; max-width: 340px; border-radius: 24px; overflow: hidden; box-shadow: 0 32px 80px rgba(0,0,0,0.2); animation: ab-slideUp 0.28s cubic-bezier(0.34,1.3,0.64,1) both; }
.ab-del-strip { height: 4px; background: linear-gradient(90deg, #dc2626, #ef4444, #fca5a5); }
.ab-del-body { padding: 28px 24px 22px; }
.ab-del-icon { width: 56px; height: 56px; border-radius: 50%; background: #fef2f2; border: 1.5px solid #fecaca; display: flex; align-items: center; justify-content: center; margin: 0 auto 16px; color: #ef4444; }
...
.ab-del-cancel { padding: 12px; border-radius: 12px; border: 1.5px solid #e2e8f0; color: #64748b; background: #fff; font-size: 0.78rem; font-weight: 700; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; transition: all 0.18s; }
.ab-del-cancel:hover { border-color: #94a3b8; color: #0f172a; }
.ab-del-confirm { padding: 12px; border-radius: 12px; border: none; background: #ef4444; color: #fff; font-size: 0.78rem; font-weight: 700; font-family: 'Plus Jakarta Sans', sans-serif; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; transition: all 0.18s; }
...
// ── Inline Alert (modal içinde) ──
const Alert = ({ type, message, onDismiss }) => {
    const icons = { error: '✕', success: '✓' };
    return (
        <div className={`ab-alert ${type}`}>
            <div className="ab-alert-icon">{icons[type]}</div>
            <div className="ab-alert-body">{message}</div>
            <button className="ab-alert-close" onClick={onDismiss} type="button">✕</button>
        </div>
    );
};
...
const AddressBook = () => {
    ...
    const [modalAlert, setModalAlert]   = useState(null); // modal-এর ভেতরে alert

    const showModalAlert  = (type, message) => setModalAlert({ type, message });
    const clearModalAlert = () => setModalAlert(null);
    ...
    const closeModal = () => { setModal(null); clearModalAlert(); };
    ...
    const handleSave = async () => {
        ...
        if (!form.firstName?.trim())     { showModalAlert("error", "Please enter first name."); return; }
        if (!form.phone?.trim())          { showModalAlert("error", "Please enter phone number."); return; }
        if (!validatePhone(form.phone))   { showModalAlert("error", "Please enter a valid phone number (e.g. 01712345678)."); return; }
        if (!form.street?.trim())         { showModalAlert("error", "Please enter street address."); return; }
        if (!form.city?.trim())           { showModalAlert("error", "Please enter city/area."); return; }
        if (!form.division)               { showModalAlert("error", "Please select a division."); return; }
        ...
        try {
            ...
            showModalAlert("error", "Failed to save address. Please try again.");
        } finally {
            setSaving(false);
        }
    };
    ...
    const f = (key, val) => { setForm(p => ({ ...p, [key]: val })); clearModalAlert(); };
                ...
                <div className="ab-body">
                    <div className="ab-top">
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            ...
                            {addresses.length > 0 && <span className="ab-top-count">{addresses.length}</span>}
                        </div>
                        <button className="ab-add-btn" onClick={() => openModal("add")}>
                                ...
                                <div key={addr.id} className={"ab-card" + (addr.isDefault ? " is-default" : "")}>
                                    <div className="ab-card-top">
                                        ...
                                        <div className="ab-card-actions">
                                            <button className="ab-action-btn" onClick={() => openModal(addr)}>Edit</button>
                                    ...
                                    {!addr.isDefault && (
                                        <button className="ab-set-default-btn" onClick={() => setDefault(addr)}>
                                            <svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><polyline points="12 8 12 12 14 14" /></svg>
                                            Set as Default
                                        </button>
                                    )}
                                </div>
                        ...
                        <div className="ab-modal" onClick={e => e.stopPropagation()}>
                            <div className="ab-modal-head">
                                <div>
                                    ...
                                    <div className="ab-modal-title">{modal === "add" ? "Add New Address" : "Edit Address"}</div>
                                </div>
                                <button className="ab-modal-close" onClick={closeModal}>
                            ...
                            <div className="ab-modal-body">
                                ...
                                {/* ── Alert — modal-এর ভেতরে ── */}
                                {modalAlert && (
                                    <Alert type={modalAlert.type} message={modalAlert.message} onDismiss={clearModalAlert} />
                                )}
                                ...
                                <div>
                                    ...
                                    <div className="ab-type-seg">
                                        {["Home", "Office"].map(l => (
                                            <button key={l} type="button" className={"ab-type-btn" + (form.label === l ? " on" : "")} onClick={() => f("label", l)}>
                                                {l === "Home" ? (
                                                    <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M3 9.5L12 3l9 6.5V20a1 1 0 01-1 1H4a1 1 0 01-1-1V9.5z" /><path d="M9 21V12h6v9" /></svg>
                                                ) : (
                                                    <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="14" rx="1" /><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2" /></svg>
                                                )}
                                                {l}
                                            </button>
                                        ))}
                                ...
                                <div>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                        ...
                                        <div className="ab-sel-wrap">
                                            ...
                                            <svg className="ab-sel-arrow" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9" /></svg>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="ab-modal-foot">
                                <button className="ab-modal-cancel" onClick={closeModal}>Cancel</button>
                                <button className="ab-modal-save" onClick={handleSave} disabled={saving}>
                                    {saving ? (
                                        <><svg className="ab-spin" width="12" height="12" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" opacity=".2" /><path fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" opacity=".8" /></svg>Saving...</>
                                    ) : (
                                        <><svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12" /></svg>Save Address</>
                                    )}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="ab-del-modal" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="ab-del-body">
                                <div className="ab-del-icon">
                                    <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                </div>
