...
.sd-root {
    ...
    animation: sd-fadeUp 0.5s cubic-bezier(0.4,0,0.2,1) both;
    margin-bottom: 80px;
}
...
.sd-refresh-btn {
    ...
    padding: 7px 14px; background: #f8fafc; border: 1.5px solid #f1f5f9;
...
.sd-stat-card:hover { border-color: #bbf7d0; box-shadow: 0 6px 20px rgba(22,163,74,0.08); transform: translateY(-2px); }
.sd-stat-label { font-size: 0.65rem; font-weight: 700; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 5px; }
.sd-stat-val   { font-size: 1.4rem; font-weight: 800; color: #0f172a; letter-spacing: -0.5px; line-height: 1; }
.sd-stat-icon  {
...
.sd-stat-card:hover .sd-stat-icon { background: #f0fdf4; border-color: #bbf7d0; color: #16a34a; }
@media (max-width: 400px) {
    .sd-stat-val { font-size: 1.1rem; }
    .sd-stat-icon { width: 34px; height: 34px; }
}
...
.sd-avatar-img {
    ...
    object-fit: cover; border: 1.5px solid #f1f5f9; flex-shrink: 0;
}
...
/* Mobile avatar — slightly same size */
.sd-mob-avatar-img {
    width: 36px; height: 36px; border-radius: 50%;
    object-fit: cover; border: 1.5px solid #f1f5f9; flex-shrink: 0;
}
.sd-mob-avatar-placeholder {
    width: 36px; height: 36px; border-radius: 50%;
    background: #f0fdf4; border: 1.5px solid #bbf7d0;
    display: flex; align-items: center; justify-content: center;
    font-size: 0.78rem; font-weight: 700; color: #16a34a;
...
/* ── Desktop review list ── */
.sd-reviews { display: flex; flex-direction: column; max-width: 100%; }
.sd-review-card {
    display: flex; align-items: flex-start; justify-content: space-between;
    gap: 16px; padding: 18px 0; border-bottom: 1.5px solid #f8fafc;
    animation: sd-fadeUp 0.4s cubic-bezier(0.4,0,0.2,1) both;
}
...
.sd-view-btn {
    display: inline-flex; align-items: center; gap: 5px;
    padding: 6px 12px; background: #f8fafc; border: 1.5px solid #f1f5f9;
...
.sd-mob-bottom {
    display: flex; align-items: center; justify-content: space-between;
    gap: 10px; padding-top: 10px; border-top: 1.5px solid #f8fafc; flex-wrap: wrap;
}
.sd-mob-product { flex: 1; min-width: 0; }
...
// ========================================
// SmartAvatar — image load না হলে initials
// ========================================
        ...
        <div className="sd-stars">
            {Array(5).fill("").map((_, j) => (
                <StarIcon
                    ...
                    fill={rating >= j + 1 ? "#16a34a" : "#e2e8f0"}
                />
            ))}
        </div>
    );

    // নাম বের করো — email হলে @ আগের অংশ নাও
                            ...
                            {data.ratings.map((review, i) => (
                                <div key={i} className="sd-review-card">
                                    <div className="sd-reviewer">
                                        {/* ✅ SmartAvatar — image না থাকলে initials */}
                                        <SmartAvatar
