...
import { MoveLeftIcon, SearchXIcon, SlidersHorizontalIcon, XIcon, ChevronDownIcon, ChevronUpIcon } from "lucide-react";
...
.shop-check-row input:checked + .shop-check-box {
...
.shop-slider-input::-webkit-slider-thumb:hover {
    transform: scale(1.18);
    box-shadow: 0 2px 10px rgba(30,58,138,0.45);
}
...
.shop-cat-count {
    ...
    background: #f8fafc; border-radius: 100px;
    padding: 1px 7px; border: 1.5px solid #f1f5f9;
}
...
.shop-pagination {
    display: flex; align-items: center; justify-content: center;
    gap: 5px; margin-bottom: 60px; flex-wrap: wrap;
}
.shop-page-btn {
    display: flex; align-items: center; justify-content: center;
...
.shop-empty {
    ...
    animation: shop-fadeUp 0.5s 0.1s cubic-bezier(0.4,0,0.2,1) both;
}
.shop-empty-card {
    text-align: center; padding: 48px 40px; background: #fff;
    border: 1.5px solid #f1f5f9; border-radius: 28px;
    box-shadow: 0 8px 32px rgba(0,0,0,0.05); max-width: 340px; width: 100%;
}
.shop-empty-icon {
    width: 68px; height: 68px; border-radius: 50%; background: #f8fafc;
    border: 1.5px solid #e2e8f0; display: flex; align-items: center;
    justify-content: center; margin: 0 auto 18px; color: #94a3b8;
}
...
@media (max-width: 768px) {
    ...
    .shop-sidebar {
        width: 100%; position: static; border-radius: 16px;
...
export default function Shop() {
    ...
    const filtered = useMemo(() => {
        return products.filter(p => {
            ...
            if (price < appliedPrice.min) return false;
            if (price > appliedPrice.max) return false;
            return true;
        });
    ...
    if (appliedPrice.min !== PRICE_MIN || appliedPrice.max !== PRICE_MAX) {
        const label = `৳${Number(appliedPrice.min).toLocaleString()} – ৳${Number(appliedPrice.max).toLocaleString()}`;
        chips.push({ label, onRemove: () => { setAppliedPrice({ min: PRICE_MIN, max: PRICE_MAX }); setPriceMin(PRICE_MIN); setPriceMax(PRICE_MAX); } });
    }
                        ...
                        <aside className={`shop-sidebar ${sidebarOpen ? 'open' : ''}`}>
                                ...
                                {[
                                    ...
                                    <label key={key} className="shop-check-row">
                                        <input
                                            ...
                                            onChange={() => setStockFilter(s => ({ ...s, [key]: !s[key] }))}
                                        />
                            ...
                            <div className="shop-filter-section">
                                ...
                                <div className="shop-slider-wrap">
                                    ...
                                    />
                                    <input
                                        type="range" className="shop-slider-input"
                                ...
                                <button
                                    ...
                                    onClick={() => setAppliedPrice({ min: priceMin, max: priceMax })}
                                >
                                    Apply
                                </button>
                            </div>
                            ...
                            <div className="shop-filter-section">
                                ...
                                <div className="shop-cat-list">
                                    {CATEGORIES.map(cat => (
                                        <div
                                            key={cat}
                                            className={`shop-cat-item ${selectedCat === cat ? 'active' : ''}`}
                                            onClick={() => setSelectedCat(selectedCat === cat ? '' : cat)}
                                        >
                                            <span>{cat}</span>
                                            {catCounts[cat] > 0 && (
                                                <span className="shop-cat-count">{catCounts[cat]}</span>
                                            )}
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="shop-chips">
                                    {chips.map((chip, i) => (
                                        <span key={i} className="shop-chip">
                                            {chip.label}
                                            <button className="shop-chip-x" onClick={chip.onRemove}>
                                                <XIcon size={9} />
                                            </button>
                                        </span>
                                    ))}
                            ...
                            {filtered.length === 0 && (
                                <div className="shop-empty">
                                    <div className="shop-empty-card">
                                        ...
                                        {(search || hasFilters) && (
                                            <button className="shop-empty-btn" onClick={() => { navigate("/shop"); clearAll(); }}>
                                                ← Clear all
                                            </button>
                                        )}
                                    </div>
                                ...
                                <div className="shop-pagination">
                                    ...
                                    {pages.map((p, i) =>
                                        p === '...'
                                            ...
                                            : <button
                                                key={p}
                                                className={`shop-page-btn ${currentPage === p ? 'active' : ''}`}
